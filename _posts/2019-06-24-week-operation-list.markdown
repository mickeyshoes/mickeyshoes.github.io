...
분류 |목표     | 진행상황     | 문제점    |    해결방안   
----- | -------------- | -------------- | -------------------- | ------------  
1 | SSA코드 병렬 실행 | 80% | 각 디바이스별 실행은 완료하였으나, profiling이 안됨. | 다른 linux pc에 visual profiler를 설치하여 jetson의 작업 상황을 보려 했으나, mpirun이 동작하지 않는 오류가 발생하여 원인을 찾고있음.. | mpirun으로 데이터가 나오지 못한다면, 각 노드별로 실행 한 결과를 합치는 방향으로 해결방안을 세움..
1 | 논문에 구체적인 틀 잡기 | 15% | profiler에서 막힘 | 우선적으로 profiler를 해결하고 구체적인 데이터를 시각화하여 자료로 사용할 수 있게 할 것
1|code 책 읽기 |18% |chapter 9까지 읽음.. |
1+|profile 분석| 10% | multi-user mode로 수행하고 싶은데 nano에서는 안됨 | NVidia 측에서 알려준 tensorflow를 설치한 방안을 수행 중.
2| code 책 읽기| | |
2|profiler에서 ssa code node별 수행 | | | 1.NVidia 질의(Nvidia에서 제공하는 dic을 보고 하였는데 안되었음..)<br>2.mpirun이 아닌 각 노드별 수행으로 데이터 모으기
